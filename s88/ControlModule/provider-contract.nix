...
let
  ...
  peerString =
    peer: name: if isNonEmptyString (peerField peer name) then peerField peer name else null;
  ...
  isGlobalUnicastIPv6 =
    value: builtins.isString value && builtins.match "^[[:space:]]*[23][0-9A-Fa-f].*" value != null;
in
{
  normalize =
    contract:
    ...
    rec {
      ...
      wanInterface = required [
        "interfaces"
        "wan"
      ];
      lanInterface = required [
        "interfaces"
        "lan"
      ];
      vpnInterface = required [
        "interfaces"
        "vpn"
      ];
      ...
      generatedConfigPath = get [ "runtime" "generatedConfigPath" ] null;
      uuidFile = required [
        "runtime"
        "uuidFile"
      ];
      ...
      profilePath =
        if profileMode == "generated-peer" then
          generatedConfigPath
        else
          required [
            "profile"
      ...
      wanIPv4RouteMetric =
        let
          m = get [ "wan" "ipv4" "routeMetric" ] null;
        in
        if m == null then null else toString m;
      wanIPv6RouteMetric =
        let
          m = get [ "wan" "ipv6" "routeMetric" ] null;
        in
        if m == null then null else toString m;
      ...
      generatedPeerForScript = {
        privateKeyFile =
          if isNonEmptyString generatedPrivateKeyFile then
            ...
            throw (
              diagnostic "WireGuard private key path required by CPM provider contract, cannot construct a default private key path"
            );
  ...
  assertions = state: [
    ...
    {
      assertion =
        state.profileMode != "generated-peer"
        || state.generatedAddresses != [ ]
        || isNonEmptyString state.generatedAddressesFile;
    ...
    }
    {
      assertion =
        ...
        || builtins.all (
          peer:
          isNonEmptyString (peerField peer "publicKey") || isNonEmptyString (peerField peer "publicKeyFile")
        ) state.generatedPeers;
    ...
    }
    {
      assertion =
        ...
        || builtins.all (
          peer:
          isNonEmptyString (peerField peer "endpoint") || isNonEmptyString (peerField peer "endpointFile")
        ) state.generatedPeers;
    ...
    }
    {
      assertion =
        state.profileMode != "generated-peer"
        || builtins.all (peer: peerList peer "allowedIPs" != [ ]) state.generatedPeers;
    ...
    {
      # The health probe target is read at runtime from the profile dnsFile
      # (the provider's in-tunnel DNS gateway); it must not be hardcoded.
      assertion = true;
      message = diagnostic "health check target is derived from the profile dnsFile";
    }
    {
      assertion =
    ...
    {
      assertion =
        state.prefixAuthority != "provider-owned-prefix" || state.providerOwnedIPv6Prefixes != [ ];
    ...
    {
      assertion = builtins.all (
        route: route.interface == state.lanInterface
      ) state.normalizedReturnRoutes;
